...
static void ev_handler(struct mg_connection *c, int ev, void *p,
  ...
  if (ev == MG_EV_MQTT_CONNACK) {
    ...
    // for mhiot POC test
    else if (json_scanf(s->p, s->len, "{mhiot: {led: %d, state: %d}}", &pin,
                   &state) == 2) {
      /* Set GPIO pin to a given state */
      mgos_gpio_set_mode(pin, MGOS_GPIO_MODE_OUTPUT);
      mgos_gpio_write(pin, (state > 0 ? 0 : 1));
      pub(c, "{type: %Q, pin: %d, state: %d}", "gpio", pin, state);
    }
    // for mhiot POC test
    else if (json_scanf(s->p, s->len, "{mhiot: {echo: %Q}}", &msg) == 1) {
      pub(c, "{mhiot: {response: %Q}}", msg);
    }
    else {
...
enum mgos_app_init_result mgos_app_init(void) {
  ...
  // set button gpio for esp8266
  mgos_gpio_set_button_handler(0, MGOS_GPIO_PULL_UP,
                               MGOS_GPIO_INT_EDGE_POS, 50, gpio_int_handler,
                               NULL);
